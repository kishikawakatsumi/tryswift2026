...
enum Tetris {
  ...
  // Bitmask of rows being cleared (bit i = row i is full)
  nonisolated(unsafe) static var clearMask: UInt32 = 0
  // Animation phase: counts down from 8 to 0 (4 blinks)
  nonisolated(unsafe) static var clearPhase: UInt8 = 0
  ...
  static func hardDrop() {
    while canPlace(curPiece, curRot, curX, curY + 1) { curY += 1 }
  }

  /// Locks piece and detects full lines. Starts animation if lines found.
  /// Returns true if animation started (caller should enter anim state).
  static func lockAndDetect() -> Bool {
    lockPiece()
    let mask = detectFullLines()
    if mask != 0 {
      clearMask = mask
      clearPhase = 8  // 4 blinks (on/off pairs)
      return true
    }
  ...
  /// Called after animation completes. Removes full lines and spawns next piece.
  static func finishClear() {
  ...
  /// Returns true if row r is part of the clear animation.
  static func isRowClearing(_ r: Int) -> Bool {
    clearMask & (1 << r) != 0
  }
  ...
  private static func detectFullLines() -> UInt32 {
    let full: UInt16 = (1 << cols) - 1
    var mask: UInt32 = 0
    for r in 0..<rows {
      if getRow(r) == full { mask |= (1 << r) }
    }
    return mask
  }

  private static func spawnNext() {
    curPiece = nxtPiece
    nxtPiece = random7()
    curRot = 0
    curX = 3
