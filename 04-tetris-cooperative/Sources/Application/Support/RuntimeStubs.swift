...
// Constructs a UInt64 from lo and hi halves using pointer manipulation.
// This avoids all 64-bit arithmetic (no shifts, no multiply).
@inline(__always)
private func makeUInt64(lo: UInt32, hi: UInt32) -> UInt64 {
  var result: UInt64 = 0
  withUnsafeMutablePointer(to: &result) { ptr in
    let p = UnsafeMutableRawPointer(ptr).assumingMemoryBound(to: UInt32.self)
    p[0] = lo  // Little-endian: lo at offset 0
    p[1] = hi  // hi at offset 4
  }
  return result
}

// Extracts lo and hi halves from a UInt64 using pointer manipulation.
@inline(__always)
private func splitUInt64(_ value: UInt64) -> (lo: UInt32, hi: UInt32) {
  var v = value
...
// MARK: - 64-bit Multiply

@c @inline(never)
func __aeabi_lmul(_ a: UInt64, _ b: UInt64) -> UInt64 {
  let (aLo, aHi) = splitUInt64(a)
  let (bLo, bHi) = splitUInt64(b)
  ...
  return makeUInt64(lo: resultLo, hi: resultHi)
}
...
func __aeabi_idivmod(_ numerator: Int32, _ denominator: Int32) -> UInt64 {
  ...
  let r = numerator &- q &* denominator
  return makeUInt64(lo: UInt32(bitPattern: q), hi: UInt32(bitPattern: r))
}

// MARK: - Memory (8-byte aligned)

@c
func __aeabi_memclr8(_ dest: UnsafeMutableRawPointer, _ count: UInt) {
  memclr(dest, count)
}
...
@c @inline(never)
func __aeabi_llsl(_ lo: UInt32, _ hi: UInt32, _ shift: UInt32) -> UInt64 {
  ...
  return makeUInt64(lo: rLo, hi: rHi)
}

@c @inline(never)
func __aeabi_llsr(_ lo: UInt32, _ hi: UInt32, _ shift: UInt32) -> UInt64 {
  if shift >= 64 { return 0 }
  var rLo: UInt32
  var rHi: UInt32
  if shift >= 32 {
...
nonisolated(unsafe) var _arcSeed: UInt32 = 0xDEAD_BEEF

@c
func arc4random_buf(_ buf: UnsafeMutableRawPointer, _ count: UInt) {
  let ptr = buf.assumingMemoryBound(to: UInt8.self)
  for i in 0..<Int(count) {
    _arcSeed ^= _arcSeed << 13
    _arcSeed ^= _arcSeed >> 17
    _arcSeed ^= _arcSeed << 5
    ptr[i] = UInt8(_arcSeed & 0xFF)
  }
}
